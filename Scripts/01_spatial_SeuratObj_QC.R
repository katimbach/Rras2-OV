...
out_dirs<- c("./Robjs", "./Plots/QC")

sapply(out_dirs, function(x) dir.create(x, showWarnings = FALSE, recursive = TRUE))
...
for (i in 1:length(samps)){
  ...
  ifelse(!dir.exists("./Plots/QC/"), dir.create("./Plots/QC/", recursive = T),
         FALSE)
  
  pdf(file= paste0("./Plots/QC/", samp, "_nCountSpatial.pdf"), width = 10, height = 7)
  ...
  pdf(file= paste0("./Plots/QC/", samp, "_nFeatureSpatial.pdf"), width = 10, height = 7)
  print(plot_grid(plot1, plot2))
  dev.off()
  ...
  pdf(file= paste0("./Plots/QC/", samp, "_LowFeatureSpatial.pdf"), width = 10, height = 7)
  print(plot_grid(plot1, plot2))
  dev.off()
  ...
  pdf(file= paste0("./Plots/QC/", samp, "_VarGeneScatter.pdf"), width = 6, height = 6)
  print(var_p1)
  dev.off()
  ...
  pdf(file= paste0("./Plots/QC/", samp, "_VarGeneSpatial.pdf"), width = 12, height = 6)
  print(var_p2)
  dev.off()
  ...
  pdf(file= paste0("./Plots/QC/", samp, "_SpatialClusters.pdf"), width = 6, height = 6)
  print(plot1)
  dev.off()
...
dir.create("./Results/Res0.8ClusterMarks", recursive=T)
...
mapply(hms, samps, FUN= function(hm, samp){
  pdf(file= paste0("./Plots/QC/", samp, "_Res0.8Heatmap.pdf"), width = 10, height = 7)
  print(hm)
  dev.off()
})